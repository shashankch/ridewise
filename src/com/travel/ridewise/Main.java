...
import com.travel.ridewise.exception.RideWiseException;
...
public class Main {
    ...
    private static void requestRide(RideService rideService, RiderService riderService) {
        if (riderService.getAllRiders().isEmpty()) {
            System.out.println("No riders registered yet. Please add a rider first.");
            return;
        }
        ...
        System.out.println("Select Preferred Vehicle Type: 1. BIKE  2. AUTO  3. CAR  4. ANY");
        int choice = Integer.parseInt(scanner.nextLine().trim());
        VehicleType preferredType = switch (choice) {
        ...
        RideRequest request = new RideRequest(riderId, distance, preferredType);
    ...
    private static void configureStrategies(RideService rideService) {
        ...
        int matchChoice = Integer.parseInt(scanner.nextLine().trim());
        rideService.setRideMatchingStrategy(matchChoice == 2 ? "LEAST_ACTIVE" : "NEAREST");

        System.out.println("Select Fare Strategy: 1. Default  2. Peak Hour");
        int fareChoice = Integer.parseInt(scanner.nextLine().trim());
        rideService.setFareStrategy(fareChoice == 2 ? "PEAK_HOUR" : "DEFAULT");
